...
// Configuramos connection_limit=1 y pool_timeout=30s para evitar que se
 ...
 *
 * Esta función reintenta la operación hasta 3 veces con backoff exponencial
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Si es error de conexión o P1001 (connection refused) o P2024 (timeout), reintentar
