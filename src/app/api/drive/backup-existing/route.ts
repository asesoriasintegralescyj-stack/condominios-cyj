...
export async function GET(request: any) {
  ...
  const stats = { proyectos: 0, ots: 0, scs: 0, fotos: 0, errores: 0 }
...
async function backupProyectos(parentFolderId: string, stats: any, limit: number) {
  ...
  for (const p of proyectos) {
    ...
    try {
      ...
      await findOrCreateFolder('Solicitudes de Compra', folderId)
      await findOrCreateFolder('Documentos', folderId)
      ...
      const data = {
        ...
        documentosCount: p.documentos ? p.documentos.length : 0,
        createdAt: p.createdAt, updatedAt: p.updatedAt, _backupDate: new Date().toISOString(),
      }

      const jsonName = codigo + ' - Datos del Proyecto.json'
      await uploadFile(jsonName, JSON.stringify(data, null, 2), 'application/json', folderId, jsonName)
...
async function backupOTs(parentFolderId: string, stats: any, limit: number) {
  ...
  for (const ot of ots) {
    try {
      ...
      let otFolderId = res.data.files ? res.data.files[0] ? res.data.files[0].id : null : null
      if (!otFolderId) {
      ...
      const data = {
        ...
        notas: ot.notas ? ot.notas.replace(/\[IDEM:[^\]]+\]\s*/g, '') : null,
...
async function backupSCs(parentFolderId: string, stats: any, limit: number) {
  ...
  for (const f of projectFolders) {
    const m = f.name.match(/^(PROY-\d+)/)
    if (m) codeToFolder[m[1]] = f.id
  }
  ...
  for (const sc of scs) {
    try {
      ...
      if (sc.origenTipo === 'Proyecto' && sc.origenCodigo) {
        const m = sc.origenCodigo.match(/^(PROY-\d+)/)
        if (m && codeToFolder[m[1]]) {
          scFolderId = await findOrCreateFolder('Solicitudes de Compra', codeToFolder[m[1]])
        }
      }
      if (!scFolderId) scFolderId = genericScFolder
      if (!scFolderId) { stats.errores++; continue }

      const mats = sc.materiales ? (function() { try { return JSON.parse(sc.materiales) } catch(e) { return [] } })() : []
      const links = sc.links ? (function() { try { return JSON.parse(sc.links) } catch(e) { return [] } })() : []

      let txt = 'SOLICITUD DE COMPRA: ' + sc.codigo + '\n'
      txt += '='.repeat(60) + '\n\n'
      txt += 'Titulo: ' + sc.titulo + '\n'
      txt += 'Descripcion: ' + (sc.descripcion || 'Sin descripcion') + '\n\n'
      ...
      txt += '--- Materiales (' + mats.length + ') ---\n'
      mats.forEach(function(m: any, i: number) {
      ...
      if (links.length) {
        txt += '\n--- Links ---\n'
        links.forEach(function(l: string, i: number) { txt += '  ' + (i+1) + '. ' + l + '\n' })
      }
      txt += '\nBackup: ' + new Date().toISOString() + '\n'

      const safeTitle = sc.titulo.replace(/[\\/]/g, '-')
      await uploadFile(sc.codigo + ' - ' + safeTitle + '.txt', txt, 'text/plain', scFolderId, sc.codigo + ' - *.txt')
      stats.scs++
