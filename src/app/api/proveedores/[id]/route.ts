...
export async function GET(
  ...
  if (session.user.rol !== 'admin' && !hasPermission(session.user.rol, 'proveedores.ver', session.userPermisos)) {
...
export async function PUT(
  ...
  const session = await getCurrentSession()
  if (!session) return apiError('No autenticado', 401)
  if (session.user.rol !== 'admin' && !hasPermission(session.user.rol, 'proveedores.editar', session.userPermisos)) {
    return apiError('Sin permisos', 403)
  }
  try {
    const { id } = await params
...
export async function DELETE(
  ...
  const session = await getCurrentSession()
  if (!session) return apiError('No autenticado', 401)
  if (session.user.rol !== 'admin' && !hasPermission(session.user.rol, 'proveedores.eliminar', session.userPermisos)) {
    return apiError('Sin permisos', 403)
  }
  try {
    const { id } = await params
